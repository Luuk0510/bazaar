<x-app-layout>
    <x-title>
        {{ __('messages.rental_advertisement') }}
    </x-title>
    <div class="py-12">
                    ...
                    <div class="flex">
                        <div class="w-1/2 pr-4">
                            <div class="bg-gray-100 dark:bg-gray-800 rounded-lg p-6">
                                ...
                                <img src="data:image/jpeg;base64,{{ $rentalAdvertisement->image }}" alt="Advertisement image"
                            ...
                            <div class="bg-gray-100 dark:bg-gray-800 rounded-lg p-6">
                                ...
                                <div class="mt-4 flex justify-between items-center">
                                    ...
                                    <div class="text-right dark:text-white">
                                        <a href="{{ route('user-profile', ['userId' => $rentalAdvertisement->user->id]) }}"
                                            class="text-blue-500 hover:underline">{{ $rentalAdvertisement->user->name }}</a>
                                    </div>
                                </div>
                                ...
                                <div class="mb-4">
                                    <form method="POST" action="{{ route('rentals.store') }}">
                                        @csrf
                                        <input type="hidden" name="rental_advertisement_id"
                                            value="{{ $rentalAdvertisement->id }}">

                                        <label for="start_date" class="block">
                                            <span
                                                class="text-gray-700 dark:text-gray-300">{{ __('messages.start_date') }}</span>
                                            <x-date-input name="start_date" id="start_date" />
                                        </label>

                                        <label for="end_date" class="block mt-4">
                                            <span
                                                class="text-gray-700 dark:text-gray-300">{{ __('messages.end_date') }}</span>
                                            <x-date-input name="end_date" id="end_date" />
                                        </label>

                                        <x-primary-button class="mt-4">
                                            {{ __('messages.rent_item') }}
                                        </x-primary-button>

                                        <input type="hidden" name="rental_advertisements_id"
                                            value="{{ $rentalAdvertisement->id }}">
                                    </form>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-700 overflow-hidden shadow-xl sm:rounded-lg">
            <div class="p-6">
                ...
                <div class="grid dark:text-gray-300 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <form id="reviewForm" action="{{ route('reviews.storeRentalReview') }}" method="POST" class="mt-4">
                    @csrf
                    <div class="grid grid-cols-1 gap-6 mt-8">
                        ...
                        <label for="comment" class="block">
                            <span
                                class="text-gray-700 dark:text-gray-300">{{ __('messages.review_description') }}</span>
